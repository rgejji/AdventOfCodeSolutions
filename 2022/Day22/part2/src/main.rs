//use graph::prelude::*;
use array2d::Array2D;
use std::fs;
use std::collections::HashMap;
use num::integer::gcd;

enum Orient {
    R,
    L,
    No
}
const image_chars: [char; 4] = ['>', 'v', '<', '^'];

#[derive(Debug)]
struct Loc(usize, usize, usize);

#[derive(Debug)]
struct Params {
    macro_cols:usize,
    scale:usize,

}

#[derive(Debug)]
struct EdgeTransition {
    face: usize,
    edge: usize,
    flip: bool,
    rotate: bool,
}

impl EdgeTransition {
    fn move_edge(&self, l:&Loc, p:&Params) -> Loc{
        //println!("Moving {:?} {:?} {:?}", self, l, p);
        let mut new_loc = Loc(l.0%p.scale, l.1%p.scale, (self.edge+2)%4);
        //println!("BEFORE FLIP: {:?}", new_loc);
        if self.flip{
            new_loc.0 = p.scale-1-new_loc.0;
            new_loc.1 = p.scale-1-new_loc.1;
        }
        //println!("BEFORE ROTATE: {:?}", new_loc);
        if self.rotate{
            let tmp = new_loc.0;
            new_loc.0 = new_loc.1;
            new_loc.1 = tmp;
        }

        //println!("BEFORE SIDE PROJECTION: {:?}", new_loc);
        match self.edge {
            0 => {new_loc.1 = p.scale-1},
            1 => {new_loc.0 = p.scale-1},
            2 => {new_loc.1 = 0},
            3 => {new_loc.0 = 0},
            _ => {println!("ERROR: encountered unexpected direction {}", new_loc.2);}
        }
        //println!("AFTER SIDE PROJECTION: {:?}", new_loc);

        let (y,x) = self.get_upper_left(p);
        //println!("Upper left is {}, {}", y, x);
        //println!("on face is {}, {}", new_loc.0, new_loc.1);
        new_loc.0 += y; 
        new_loc.1 += x;
        new_loc
    }

    fn get_upper_left(&self, p:&Params) -> (usize, usize){
        let macro_x = self.face%p.macro_cols;
        let macro_y = (self.face-macro_x)/p.macro_cols;
        (macro_y*p.scale, macro_x*p.scale)
    }
}


fn setup_edge_moves() -> HashMap<(usize,usize), EdgeTransition> {
    let mut map = HashMap::new();

    //test case edge transitions
    /*
    add_edge_transitions((2,0),(11,0), &mut map);
    add_edge_transitions((2,2),(5,3), &mut map);
    add_edge_transitions((2,3),(4,3), &mut map);
    add_edge_transitions((4,1),(10,1), &mut map);
    add_edge_transitions((4,2),(11,1), &mut map);
    add_edge_transitions((5,1),(10,2), &mut map);
    add_edge_transitions((6,0),(11,3), &mut map);
    */
    add_edge_transitions((1,2),(6,2), &mut map);
    add_edge_transitions((1,3),(9,2), &mut map);
    add_edge_transitions((2,0),(7,0), &mut map);
    add_edge_transitions((2,1),(4,0), &mut map);
    add_edge_transitions((2,3),(9,1), &mut map);
    add_edge_transitions((4,2),(6,3), &mut map);
    add_edge_transitions((7,1),(9,0), &mut map);
    map
}




//no flips (0,1), (3,2)
//flips (0,3), (1,2)

fn has_flip(a:usize, b:usize) -> bool {
    if a==b{
        return true
    }
    if (a%2) == 0 && b==(4-1+a)%4 {
        return true
    } 
    if (b%2) == 0 && a==(4-1+b)%4 {
        return true
    }
    false
}


fn add_edge_transitions(face_a:(usize,usize), face_b:(usize,usize), map:&mut HashMap<(usize,usize), EdgeTransition>) {
    let flip = has_flip(face_a.1, face_b.1);
    let rotate = face_a.1 %2 != face_b.1 %2;
    map.insert((face_a.0, face_a.1), EdgeTransition {face: face_b.0, edge: face_b.1, flip:flip, rotate:rotate});
    map.insert((face_b.0, face_b.1), EdgeTransition {face: face_a.0, edge: face_a.1, flip:flip, rotate:rotate});
}


fn read_input(input:&str)-> (Array2D<char>,Vec<(usize,Orient)>){
    let lines:Vec<&str> = input.split("\n").collect();
    let n_rows = if lines[lines.len()-2].is_empty() {lines.len()-2} else {lines.len()-3};
    let n_cols = lines[0].len();
    println!("Making array of size {}x{}", n_rows, n_cols);
    let mut arr = Array2D::filled_with(' ', n_rows, n_cols);

    let mut instructions = vec![];
    let mut curr_val:usize = 0;
    println!("instructions line: {}", lines[n_rows+1]);
    for  c in lines[n_rows+1].chars() {
        match c{
            'R' => {
                instructions.push((curr_val, Orient::R));
                curr_val = 0;
            },
            'L' => {
                instructions.push((curr_val, Orient::L));
                curr_val = 0;
            },
            _ => curr_val = curr_val*10 + c.to_digit(10).unwrap() as usize,
        }
    }
    if curr_val != 0{
        instructions.push((curr_val, Orient::No));
    }

    
    for (i,line) in lines.into_iter().enumerate() {
        if i>= n_rows{
            break;
        }
        for (j,c) in line.chars().into_iter().enumerate() {
            arr[(i,j)] = c;
        }
    }
    println!("Read in {} instructions", instructions.len());

    (arr, instructions)
}

fn get_start_loc(arr: &Array2D<char>) -> Loc{
    for i in 0..arr.num_columns(){
        if arr[(0,i)] == '.'{
            return Loc(0,i,0)
        }
    }
    println!("Error: could not find an empty spot");
    return Loc(0,0,0)
}

fn perform_move(arr: &mut Array2D<char>, instruction:&(usize,Orient), loc:&mut Loc, edge_moves:&HashMap<(usize,usize), EdgeTransition>, p:&Params) {
    //attempt forward move until we cannot
    for _i in 0..instruction.0{
        let(valid, new_loc) = try_forward_move(arr, loc, edge_moves, p);
        if valid{
            arr[(loc.0,loc.1)] = image_chars[loc.2];
            *loc = Loc(new_loc.0, new_loc.1, new_loc.2);
        } else {
            break;
        }
    }
    //apply rotate
    match instruction.1 {
        Orient::R => loc.2 = (loc.2+1)%4,
        Orient::L => loc.2 = (4+loc.2-1)%4,
        Orient::No =>{},
    }
    //println!("ROTATEED TO {:?}", loc);
}

fn try_forward_move(arr: &Array2D<char>, loc:&Loc, edge_moves:&HashMap<(usize, usize), EdgeTransition>, p:&Params) -> (bool, Loc){
    //Check for jumps as we move
    let mut next_loc = Loc(loc.0, loc.1, loc.2);
    let mut has_jump = false;
    match loc.2 {
        0=> if loc.1 == arr.num_columns()-1 { has_jump = true } else { next_loc.1 += 1},
        1=> if loc.0 == arr.num_rows()-1 { has_jump = true } else { next_loc.0 += 1},
        2=> if loc.1 == 0 { has_jump = true } else { next_loc.1 -= 1},
        3=> if loc.0 == 0 { has_jump = true } else { next_loc.0 -= 1},
        _=> println!("Error: encountered unexpected orientation {}", loc.2),
    }
    //println!("NEW LOC {:?}, nr {}, nc {}", next_loc, arr.num_rows(), arr.num_columns());

    let mut next_val = arr[(next_loc.0, next_loc.1)];
    if has_jump || next_val == ' '{
        let face = get_face(loc, p);
        //println!("At loc {:?} with face {}", loc, face);
        next_loc = match edge_moves.get(&(face, loc.2)) {
            Some(val) =>{
                //println!("Executing transition {:?}", val);
                val.move_edge(loc, p)
            },
            None => {
                println!("Error, could not recover transition ({},{})", face,loc.2);
                Loc(0,0,0)
            }
        };
        next_val = arr[(next_loc.0, next_loc.1)];
    }

    if next_val == ' '{
        println!("ERROR: WE ARE AT A SPACE")
    }
    (next_val != '#', next_loc)
}

fn get_face(l: &Loc, p:&Params) -> usize{
    l.1/p.scale + p.macro_cols*(l.0/p.scale)
}

fn print_arr(arr: &Array2D<char>){
    for i in 0..arr.num_rows(){
        for j in 0..arr.num_columns(){
            print!("{}", arr[(i,j)]);
        }
        println!("");
    }
}


fn main() {
    let input = if INPUT.is_empty() { fs::read_to_string("input.txt").unwrap() } else { INPUT.to_string() };

    let (mut arr,instructions) = read_input(&input);
    let mut curr_loc = get_start_loc(&arr);
    let scale = gcd(arr.num_rows(), arr.num_columns());
    let state = Params{
        scale: scale,
        macro_cols: arr.num_columns()/scale,
    };
    let edge_moves = setup_edge_moves();

    println!("Starting at location {:?}", curr_loc);
    for (cnt, instruction) in instructions.into_iter().enumerate() {
        perform_move(&mut arr, &instruction, &mut curr_loc, &edge_moves, &state);
        //println!("Curr position at t={}, {:?}", cnt, curr_loc);
    }
    arr[(curr_loc.0,curr_loc.1)] = '*';
    print_arr(&arr);
    println!("Final position {:?}", curr_loc);
    println!("Password is {}", 1000*(curr_loc.0+1)+4*(curr_loc.1+1)+curr_loc.2);
}

const INPUT:&str = "";

/*
const INPUT:&str = "        ...#    
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5";
*/

/*
const INPUT:&str = "                                                  ..................#..#.....#....................#............#.......#..##..........................
                                                  .#...#.....#.........................................#...##...#...#....#...................#........
                                                  ...........##.......#.........#.#.#...#..#.............#.............#........#.................##..
                                                  ..#........#.......#.............................................#......#...#.......................
                                                  ..............................#..........#...................#..................#.............#.....
                                                  .......#..#.....#.......#...#.........................#.........#...#..#...............#........##..
                                                  .........#.....#...............#...##.........#...................#..#..##.....#...........#.....#..
                                                  ..#.#.......#............#.#.............#...................#......................##..............
                                                  #......#......#......................#.......#.........#..............#.....#.#....#...#..#.........
                                                  #.#.......#.............................#.........................#.........#.#..#............#.....
                                                  #.................#...........#...#...#.....................................#............##...#.#...
                                                  ............#.....##............................##............#..#...#.......#..#..............#....
                                                  ....##..............#.................#...............#....#..#...#..............#.........#........
                                                  ......#..#...................................#.#......#..#....#............#..#......#..............
                                                  .........#........................................#........#....##....................#...........#.
                                                  .............#....##.#..#...#...##.#..................#.........#.##................................
                                                  ...............#...........#.........##................#............................................
                                                  #.###.#..#.........#......#........#......#......#.......................#.....................#....
                                                  ......................#................#.........#....#..##.....##....#....#........................
                                                  ...#.#.............................#.....##...#.#......#.......#.#..........#.###................#..
                                                  ......#....#.............#...............#.........#................#..#...........#......##......#.
                                                  .#.....................#...........#..............#...........................................##....
                                                  ....#...................................#...........................#..............#...........#....
                                                  ......................................#..........................#.............##...............#...
                                                  ###.......#.....#..#...........#..#.#.....#......#..........#........#.....#..#......#..............
                                                  .................#....#.............................#..................#.##.....#...................
                                                  ...............#...........................#....#......#..#...........#.............................
                                                  .........#....#........#.....................#.......#.#.#................#.........................
                                                  .........##..#....................................#.........#...#...#.....#.#......#................
                                                  ...##.......................#...........#...#........#...#.###....................#..........#......
                                                  #....................#......#...#....................#................#.#........##.........#......#
                                                  .....................................#................#..........#......#.........#...#.............
                                                  .....................#.....#..........................................#............#................
                                                  ....#..#..#........#..........#....................#....#...#..#..............##................#...
                                                  ............#.#...........##.............................#.............#..#.................#.......
                                                  .....#........#...##...............................#............#...#....#...#............##........
                                                  .....#...#.......#....#....#....#...................#...........#................#.........#........
                                                  .#.#.......#.#..........#..................#.........#....#...............................#.........
                                                  .........#..#.............#......#......#......#........#........#.#.#.................#....##......
                                                  .........#....#......................#.#.............###...#...#........#.......#....#..##..........
                                                  ...................##......#.........#...#...................#..................#...................
                                                  ..........#..........#....#.........#...................#............#..#...........................
                                                  ..........................#...............#....#...........#.................#.........#.#...#.#....
                                                  ......#....#...................#........................#........#......#...#......#................
                                                  #..................................#....#................#.#..............#.........................
                                                  .....#...#......................................#.....................#.....#......#........#.....#.
                                                  ..#.....#.......#..#...#...............................#.......#..................#......#.#........
                                                  ....................#.......#........#.........................##..#.........#.#.#........#....#....
                                                  ........#............#.......................................#........##............................
                                                  ....#..............#...........#....##............#.............#.....................#.............
                                                  .#........#.........#.....#.#...#..............#.#
                                                  ..#......#....................#.##........#...#...
                                                  ............#.......##......##........#...........
                                                  ......#.#.........#.......................#.......
                                                  ............#...................................#.
                                                  ............................#.#..............#....
                                                  .........................................#........
                                                  ............#.......#....#..#................#....
                                                  ...........#.#............#.....................#.
                                                  ..........#...#.............................#.....
                                                  ....................................#.#..##.......
                                                  ..#....#............#.....#.......................
                                                  .....#...#...........#............................
                                                  ...............#.......#..........#.....#.........
                                                  ..................................................
                                                  .....#.##.......#.##.....#.#.#...#................
                                                  ...........#..##.....#..................#.........
                                                  ....................#.......................#.....
                                                  .......#...##...............................#...##
                                                  ......#..............#........#.........#....#....
                                                  #.#.##...........#.......#..#........#...........#
                                                  ............##.................#.##...........#...
                                                  ...#.......#................#.#..#.#..............
                                                  ..#...........#............#..#.#......#.........#
                                                  ..................................................
                                                  ...........#..............#..............#........
                                                  ....#.#..............##.......#.#......#...#.#....
                                                  .........##.............#...#...#.................
                                                  ...#......#.....#...#..#..##..........#...........
                                                  .........#............#...........................
                                                  ......................#....#.........#............
                                                  .......................#...............#.......#..
                                                  ..........#.#....#............##................##
                                                  ..#......#..........#..#......#.......#.........#.
                                                  ................#.#...#.......#........#..........
                                                  ....#.....#..............................#........
                                                  ..............#............#...................#..
                                                  ....#.......#..#.....#...........#.....#......#...
                                                  ......#.....................#......#.#............
                                                  .....#.#....................#..........##.....#...
                                                  #...........#........#............................
                                                  ..#..........#..#...........#....................#
                                                  ...#.#.........#.........#.....#............#.....
                                                  ...#.........................#....#.#......#......
                                                  ....#.....#..............#.............#..........
                                                  ...#........#...........##.......................#
                                                  #....#.......#......#...#..........#..........#...
                                                  ........#...#.......#.....#...#................#..
                                                  #..............#.#....#..#...................#....
                                                  .........#.....#.#................................
#....##................#..........#...#.#......#...........................#.........#...........#..
#..........................#..........................#......#.......##.#...........................
#.......##...#.......#........#.....#......#...#.#....#.....#.......#.............#.................
.#......##.....#.#.........#........#...#.......................................#...........#.....#.
........#..###............#......##..#............#.......#.....#....#.......#......................
....#.#.....#...#....#.....#...#..#.#...........#..............#..................#............#....
..................#........#..#.....#..#...#....##..............#..##...#.......#.....#.............
......#...#.##......#......#.....#....#..........#.....#.....#...............................##.....
.......................#.....##.......#....#....................................#.................#.
.........#........................#..........#...#......#.....##....................................
.......#........#...................#.......#...#.............#.#..........#.#...#..#.....#.....#...
.....#..............#................#.....#........................#....#....#.....................
##...#.#..#..........#.................#....##......#.......#..................#.......#..#..#......
#............#...#.................#.................#.#..#.......#.#...............#...............
.#...........#...#.........................#........#.......##..................#....#..............
#......#.........#...................#........#........#.........#.............#.........#..........
.................................#...#......................#........#...#....#.....................
#..........#...........................#.............#....##.............###.........#........#.....
..............#..##...#..............#.......#............#...#.....#............#....#......#.#....
.........#.....#......#.....................................#.............#.......##..#.......#.....
..#..#.#......................#....#..........#...........#............................#....#...#...
#............#..............#...#...##.#.##........#..#....#....#...#...............#.#.............
..#...............#....#...#.........#..#......#..............#........#.........#..................
...........#....................#.....#...#...#..........#..........................................
......................#.............#....#..............#.......#....#.#...........................#
.................#.......#...#.......#...................#.#.......#.....#..........................
#............................#..#..........#..............##..#.............#..........#............
..................#.....................................#..#.#................................#....#
.#......#......#.......#........#.............#..#....................................#.....#..#....
........#..#.......................................#............#.....##.........#...........#..#...
......#..............#..#..#..#.............#..................#...#...........................#....
......................#...#..#.......#.#....................#...........#.......................#...
..#...........#..................#..#....#............................#....#...#.............##..#..
......#....#....#.#.......#...#...............#..........#..........#....#...#....................#.
......................................#................................#...#...........#....#..#....
................#.......................#.......................##.#........#...#............#......
......#..#.................#..........#......................#............#....#..#.......#.........
..##..............................#..#....#.................#........#.#.......#....................
...##.#....#..#..............#......#.......#..#.#.#.......#.#.......##..........#.....#..........##
......#..#....#....#..#.......#............#........###..........#.#..#...........#..........#..#..#
...#........#..........#....................#.........#.....................#...................#...
.#.#........#.................................#..............................#......#...............
.........#...........##........#..............##........#..........#.................#..............
......###....##...#.......................#....................#...................##.#..........#..
..............#....#........................................#.#.............#.....#.#....##.......#.
.........................................................................#.#.##.#..#.....#..........
...#.........#.........#......#..................#....................#..........................#..
..#...#.....#.............#..##..............................#.........##.#..#......................
.....................#...........#.....#......................#..............##..........#.......##.
.............#...............##.................#...#.........##........##.........................#
...#..............................................
.#.#..#...........#.............#.#......##......#
.................................#........##..#.#.
.......................#......#.........#..#......
..#.............#.#................#..............
.........##.##....................................
......#..##.............#....#.........#..........
...............#...........#..............##......
.........#.......#.#..#................#.....#..#.
.#..#.......................#.....#.........#..#..
.................#..#...................##.#..#...
........#...............#...##.............#..##..
...............#..........#.......................
..........#......#..#...................#.......#.
.#.......#..#..........................#..#.......
#.......#............................#............
.....#.#........#.........###............#........
.#.............#............##..#...#...#.#.......
.........#.......#..........#...#........#.#......
...#...........#...............#..................
........#................................#........
............#................................#....
....#........................#..##......#...#.....
.#.##..............#.#..................#.#...#.#.
.............#..........#...................#.....
..##....#....#..#...........#....#.##.............
.#.........#....................#...............#.
........#.#..........#...............#...#...#...#
...............#.#..................#.....##......
.....#....#.##..........#.......#....#...#....#.#.
..#.............#......#............#......#..#...
..#..#............#.#................#..#.........
.........#.......#......#......................#..
..........#.........................#.............
.##.......#...................#.........#..#......
...........#.....##.....#.#...#..........###......
...........#.......#..............................
.........#.......#...#.....#..........#....#..#...
......###.#....#.#..........##....#.......##.#....
.......#.............#..................#.....#...
.............#..........#......#......#...........
...#...........###.....#..#.................#.#...
..................#........##....#......#..#......
........................#.........##.....##.......
........#..#.#....#..........#...#...........#....
.....#........##........#............#..#.........
....................#............#.......#........
.................#.#...#....#.........#..##.......
...........#..........##....#..........#.....#....
..............#..........#.............#..........

";
*/
