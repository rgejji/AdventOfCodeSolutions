...
fn main() {
	...
	let mut START: Pos = get_start(&board);
    ...
    let mut result:usize = 9999;
    for i in 0..board.num_rows(){
        for j in 0..board.num_columns(){
            if board[(i,j)] == ASCII_a{
                //println!("Testing {}, {}", i, j);
                match bfs(&Pos(i.try_into().unwrap(), j.try_into().unwrap()), |p| p.successors(&board), |p| *p == GOAL){
                    None => {},
                    Some(i) => {
                        let test = i.len()-1;
                        if test < result{
    ...
    println!("Best path took {} steps", result);
