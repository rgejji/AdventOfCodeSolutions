...
func calculate() {
	for i := 0; i+3 < len(values); i += 4 {
		...
		c := values[i+3]
		if a < 0 || a >= len(values) {
			fmt.Printf("Error: Read loc A at i=%d is beyond bdry\n", i+1)
			break
		}
		if b < 0 || b >= len(values) {
			fmt.Printf("Error: Read loc B at i=%d is beyond bdry\n", i+2)
			break
		}

		if c < 0 || c >= len(values) {
			fmt.Printf("Error: Write location at i=%d is beyond bdry\n", i+3)
			break
		}
...
func main() {
	readInput()
	for noun := 1; noun < len(values); noun++ {
		for verb := 1; verb < len(values); verb++ {
			readInput()
			modifyInput(noun, verb)
			calculate()
			if values[0] == 19690720 {
				fmt.Printf("Found result with noun=%d, verb=%d, score = %d\n", noun, verb, 100*noun+verb)
				return
			}
