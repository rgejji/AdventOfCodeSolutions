...
func findSleepyHead() (int, int) {
	maxSleep := 0
	guardID := -1
	for id, guard := range guardLogs {
		if maxSleep < guard.timeSlept {
			maxSleep = guard.timeSlept
			guardID = id
		}
	}
	return guardID, maxSleep
}

func findMaxMinute(id int) int {
	maxMinute := -1
	maxCnt := 0
...
func main() {
	...
	id, timeSlept := findSleepyHead()
	//Lookup count
	maxMin := findMaxMinute(id)
	fmt.Printf("Guard %d slept for %d with maxMin %d\nAnswer: %d\n", id, timeSlept, maxMin, id*maxMin)
