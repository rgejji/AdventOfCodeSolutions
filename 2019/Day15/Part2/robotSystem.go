...
func main() {
	...
	idA := graphutil.GetID(grid, oxygen.y, oxygen.x)
	maxPathLength := int64(0)
	pathTree := graphutil.GetAllShortestPaths(g, idA)
	nodes := g.Nodes()
	for nodes.Next() {
		n := nodes.Node()
		idB := n.ID()
	...
	fmt.Printf("Shortest path is from id %v has distance: %v\n", idA, maxPathLength)
}
