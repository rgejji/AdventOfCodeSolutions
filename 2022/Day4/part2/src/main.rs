...
fn score_line(s1:&str) -> u32 {
    ...
    //Check if one segement is disjoin to the left of the other. 
    if seg_a.start < seg_b.start && seg_a.end < seg_b.start {
        return 0;
    }
    if seg_b.start < seg_a.start && seg_b.end < seg_a.start {
        return 0;
    }
    //Otherwise overlap occurs
    1
}
