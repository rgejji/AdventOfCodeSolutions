...
func readInput() {
	...
	for _, row := range rowSlice {
		row = strings.TrimLeft(row, "position=<")
		strVec := strings.Split(row, " velocity=<")
		locParts := strings.Split(strVec[0], ",")
		velParts := strings.Split(strVec[1], ",")
		y, err := strconv.Atoi(strings.TrimSpace(locParts[0]))
		if err != nil {
			fmt.Printf("Unable to parse x from '%s': %s\n", locParts[0], err.Error())
		}
		x, err := strconv.Atoi(strings.Trim(locParts[1], "> "))
		if err != nil {
			fmt.Printf("Unable to parse y from '%s': %s\n", locParts[1], err.Error())
		}
		vy, err := strconv.Atoi(strings.TrimSpace(velParts[0]))
		if err != nil {
			fmt.Printf("Unable to parse vx from '%s': %s\n", velParts[0], err.Error())
		}
		vx, err := strconv.Atoi(strings.Trim(velParts[1], "> "))
		if err != nil {
			fmt.Printf("Unable to parse vy from '%s': %s\n", velParts[1], err.Error())
		}
...
func writeGrid() {
	...
	currGrid = make([][]bool, maxx-minx+1, maxx-minx+1)
	for i := 0; i < maxx-minx+1; i++ {
		currGrid[i] = make([]bool, maxy-miny+1, maxy-miny+1)
	}

	for _, p := range points {
		currGrid[p.x-minx][p.y-miny] = true
	}
}
...
func scoreGrid() int {
	...
	for i, p1 := range points {
		for j, p2 := range points {
			if i != j && (Abs(p1.x-p2.x) == 0 || Abs(p1.y-p2.y) == 0) {
				score++
			}
...
//Abs returns the integer absolute value
func Abs(x int) int {
	if x < 0 {
		return -x
	}
...
func main() {
	...
	for i := 1; i < maxWait; i++ {
		...
		score := scoreGrid()
		if score > bestScore {
			fmt.Printf("Found score %d at iteration %d\n", score, i)
			bestScore = score
			writeGrid()
			if len(currGrid) < 1000 {
				printGrid()
			}
		}
	}
	fmt.Printf("Best score is %d", bestScore)
}

/*
const inputStr = `position=< 9,  1> velocity=< 0,  2>
...
const inputStr = `position=< 21518, -21209> velocity=<-2,  2>
