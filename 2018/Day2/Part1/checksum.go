...
//getLetterCounts gets the number of counts per rune
func getLetterCounts(word string) map[rune]int64 {
	frequMap := make(map[rune]int64)

	//count each rune
	for _, r := range word {
		val, found := frequMap[r]
		if !found {
			frequMap[r] = 1
		} else {
			frequMap[r] = val + 1
...
func main() {
	...
	numFoundCounts := make([]int, maxNum, maxNum)
	for _, word := range rowSlice {
		foundCounts := make([]bool, maxNum, maxNum)
		//get counts per letter in the word
		counts := getLetterCounts(word)
		fmt.Printf("Counts are: %+v\n", counts)

		//Determine if we found a two or three
		for _, count := range counts {
			if count < maxNum {
				foundCounts[count] = true
			}
		}

		//Increment numFoundCounts
		for count, found := range foundCounts {
			if found {
				numFoundCounts[count]++
			}
		}
	}

	fmt.Printf("Checksum is %d\n", numFoundCounts[2]*numFoundCounts[3])
}
