...
func main() {
	...
	maxStrength = recurseBridge([]port{}, 0, 0)
	fmt.Printf("found max strength %d\n", maxStrength)
}

func recurseBridge(currentBridge []port, currStrength int, openLink int) int {
	//find open link
	...
	for _, port := range possiblePorts {
		...
		//Recurse
		foundStrength := recurseBridge(currentBridge, currStrength+port.A+port.B, newOpenLink)
		if foundStrength > bestStrength {
			bestStrength = foundStrength
		}
