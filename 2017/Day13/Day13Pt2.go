...
func main() {
	...
	delay := 0
	for {
		severity := getSeverityForDelay(layerArray, delay)
		if severity == 0 {
			fmt.Printf("The desired delay is %d\n", delay)
			break
		}
		delay++
		if delay%100000 == 0 {
			fmt.Printf("Encountered delay %d\n", delay)
		}
	}

}

//If we are caught we increase the severity by 1
func getSeverityForDelay(layerArray []int, delay int) int {
	//Perform simulation where i = time
	severity := 0
	for i, maxDepth := range layerArray {
		if 0 == getDepth(maxDepth, i+delay) {
			severity++
		}
	}
	//fmt.Printf("Severity is for delay %d is %d\n", delay, severity)
	return severity
}
