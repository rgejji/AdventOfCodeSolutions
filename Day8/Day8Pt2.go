...
func main() {
	...
	registerMap = make(map[string]int64)
	for _, row := range rows {
		name, val := setRegisterValue(row)
		if val > max {
			max = val
			maxName = name
		}
	}
	fmt.Printf("Max is found with name %s and value %d\n", maxName, max)
}

func setRegisterValue(line string) (string, int64) {
	zero := int64(0)
	instructionSplit := strings.Fields(line)
	if len(instructionSplit) != 7 {
		fmt.Printf("Unable to parse string %s of invalid size", instructionSplit)
		return "", zero
	}

	loc := instructionSplit[0]
	instruction := instructionSplit[1]
	updateValueStr := instructionSplit[2]
	compareToLoc := instructionSplit[4]
	comparisonOp := instructionSplit[5]
	checkValueStr := instructionSplit[6]

	updateValue, err := strconv.ParseInt(updateValueStr, 10, 64)
	if err != nil {
		fmt.Printf("Unable to parse %s\n", err.Error())
		return "", zero
	}

	checkValue, err := strconv.ParseInt(checkValueStr, 10, 64)
	if err != nil {
		fmt.Printf("Unable to parse %s\n", err.Error())
		return "", zero
	}
	value := getValue(loc)
	compareToValue := getValue(compareToLoc)

	//Check if condition works, otherwise return
	valid := false
	switch comparisonOp {
	case "==":
		if compareToValue == checkValue {
			valid = true
		}
	case "!=":
		if compareToValue != checkValue {
	...
	default:
		fmt.Printf("UNRECOGNIZED COMPARISON OP %s\n", comparisonOp)
	}
	if !valid {
		return "", zero
	}
	...
	switch instruction {
	case "inc":
		registerMap[loc] = value + updateValue
	case "dec":
		registerMap[loc] = value - updateValue
	default:
		fmt.Printf("UNRECOGNIZED INSTRUCTION %s\n", instruction)
	}
	return loc, registerMap[loc]
}

func getValue(loc string) int64 {
	if val, ok := registerMap[loc]; ok {
		return val
	}
	registerMap[loc] = int64(0)
	return int64(0)
}

const (
	/*inputStr = `b inc 5 if a > 1
	a inc 1 if b < 5
	c dec -10 if a >= 1
	c inc -20 if c == 10`*/
	inputStr = `av inc 167 if f > -9
	av inc 640 if uea == 0
	fk dec -960 if tn > -9
