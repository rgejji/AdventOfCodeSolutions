...
fn get_start_loc(arr: &Array2D<char>) -> (usize,usize,usize){
...
fn perform_move(arr: &mut Array2D<char>, instruction:&(usize,Orient), loc:&mut (usize,usize,usize)) {
    //attempt forward move until we cannot
    for _i in 0..instruction.0{
        let(valid, new_loc) = try_forward_move(arr, loc);
        if valid{
            arr[(loc.0,loc.1)] = image_chars[loc.2];
            *loc = (new_loc.0, new_loc.1, new_loc.2);
        } else {
            break;
        }
    }
...
fn try_forward_move(arr: &Array2D<char>, loc:&(usize, usize, usize)) -> (bool, (usize,usize,usize)){
    let mut next_loc = (loc.0, loc.1, loc.2);
    let mut delta_x = 0;
    let mut delta_y = 0;
    match loc.2 {
        //test right move
        ...
        //test left move
        2 => {
            delta_x = arr.num_columns() - 1;
        }
        //test up move
        3 => {
            delta_y = arr.num_rows() - 1;
        }
        _ => {
            println!("Error: encountered unexpected orientation {}", loc.2);
        }
    }
    next_loc.1 = (delta_x + next_loc.1) % arr.num_columns();
    next_loc.0 = (delta_y + next_loc.0) % arr.num_rows();
   
    //Check for jumps.
    while arr[(next_loc.0, next_loc.1)] == ' '{
        next_loc.1 = (arr.num_columns() + delta_x + next_loc.1) % arr.num_columns();
        next_loc.0 = (arr.num_rows() + delta_y + next_loc.0) % arr.num_rows();
    }

    let next_val = arr[(next_loc.0, next_loc.1)];
    (next_val != ' ' && next_val != '#', next_loc)
}
...
fn main() {
    ...
    for (cnt, instruction) in instructions.into_iter().enumerate() {
        perform_move(&mut arr, &instruction, &mut curr_loc);
