//If this doesn't work, I should do priority queue with pruning seen or symmetrically seen results
...
impl State { 
  fn successors(&self, flows:&Flows) -> Vec<(State, i32)> {
    ...
    let e_loc = self.e_pos.clone();
    let neighbors = flows.graph.out_neighbors(self.pos).as_slice();
    let e_neighbors = flows.graph.out_neighbors(self.e_pos).as_slice();
    let mut valid = vec![];


    

    //Check if me opening valve is beneficial
    if !self.valve_state.contains(&loc) && flows.rates[&loc] > 0 { 
        //Check if elephant opening is also beneficial
        if !self.valve_state.contains(&e_loc) && flows.rates[&e_loc] > 0 { 
            let mut new_valve_state = self.valve_state.clone();
            new_valve_state.insert(loc);
            new_valve_state.insert(e_loc);
            let new_valve = State {
                pos: loc.clone(),
                e_pos: e_loc.clone(),
                valve_state: new_valve_state,
                time: self.time+1,
                pressure_delta: self.pressure_delta+flows.rates[&loc]+flows.rates[&e_loc],
                pressure_total: self.pressure_total+self.pressure_delta
            };
            let edge_weight:i32 = penalty - new_valve.pressure_delta;
            valid.push((new_valve, edge_weight));
        }

        for e_neighbor in e_neighbors {
            let mut new_valve_state = self.valve_state.clone();
            new_valve_state.insert(loc);
            let new_valve = State {
                pos: loc.clone(),
                e_pos: e_neighbor.clone(),
                valve_state: new_valve_state,
                time: self.time+1,
    ...
    //See if moving somewhere is good
    for neighbor in neighbors {
        //see if elephant opening valve is also good
        if !self.valve_state.contains(&e_loc) && flows.rates[&e_loc] > 0 { 
            let mut new_valve_state = self.valve_state.clone();
            new_valve_state.insert(e_loc);
            let new_valve = State {
                ...
                valve_state: new_valve_state,
                time: self.time+1,
                pressure_delta: self.pressure_delta+flows.rates[&e_loc],
                pressure_total: self.pressure_total+self.pressure_delta
            };
            let edge_weight:i32 = penalty - new_valve.pressure_delta;
            valid.push((new_valve, edge_weight));
...
fn main() {
    ...
    let begin:State = State {
        ...
        e_pos: stoi("AA".to_string()),
        valve_state: HashSet::new(),
        time: 0,
        pressure_delta: 0,
        pressure_total: 0,
    };
    let result = dijkstra(&begin, |p| p.successors(&f), |p| p.time == end_time).expect("Error, no short path found");
