...
func main() {
	...
	tree, err := getTreeFromDataInput(inputStr)
	if err != nil {
		log.Fatalf("UNABLE TO CONSTRUCT TREE: %s\n", err.Error())
	}
	rootTree := getRoot(tree)
	fmt.Printf("HAVE ROOT %s\n", rootTree.Name)
	calculateAndValidateTotalWeight(rootTree)

	//From the output look at the two that are different and subtract from the value to get the correct answer

	fmt.Println("vim-go")
}

func getTreeFromDataInput(input string) (*Tree, error) {
	currTree := &Tree{}
	childTree := &Tree{}
	rows := strings.Split(input, "\n")
	for cnt, row := range rows {
		fmt.Printf("Looking at row %s\n", row)
		ok := false

		valsSlice := strings.Split(row, " ")
		if len(valsSlice) < 2 {
			return nil, fmt.Errorf("Unable to extract enough values in row %d", cnt)
		}
		name := sanitize(valsSlice[0])
		weightStr := strings.Trim(valsSlice[1], "()")
		...
		//Check if tree already exists. If not, create it
		if currTree, ok = treeMap[name]; !ok {
			currTree = &Tree{Name: name, SubTrees: []*Tree{}}
			treeMap[name] = currTree
		}
		currTree.Value = weight

		//add tree sub nodes if available
		if len(valsSlice) >= 4 {
			for _, childNameUnprocessed := range valsSlice[3:] {
				childName := sanitize(childNameUnprocessed)
				if childName == "" {
					continue
				}
				//Add child and set parent and child fields for sub node
				if childTree, ok = treeMap[childName]; !ok {
					childTree = &Tree{Name: childName, SubTrees: []*Tree{}}
					treeMap[childName] = childTree

				}
				childTree.Parent = currTree
				//fmt.Printf("Setting %s to have parent %s\n", childTree.Name, currTree.Name)
				currTree.SubTrees = append(currTree.SubTrees, childTree)
			}
		}
	}
	return currTree, nil
...
func getRoot(someNode *Tree) *Tree {
	if someNode.Parent == nil {
		return someNode
	}
	return getRoot(someNode.Parent)
}

func calculateAndValidateTotalWeight(currTree *Tree) int64 {
	totalValue := currTree.Value
	subTreeValue := int64(0)
	baseName := ""

	for i, childTree := range currTree.SubTrees {
		childTotalValue := calculateAndValidateTotalWeight(childTree)
		if i == 0 {
			subTreeValue = childTotalValue
			baseName = childTree.Name
			totalValue += childTotalValue
			continue

		}
		if subTreeValue != childTotalValue {
			fmt.Printf("FOUND MISMATCH IN STACK NAME %s of SCORE %d VS OTHER STACK %s WITH SCORE %d\n", childTree.Name, childTotalValue, baseName, subTreeValue)
		}
		totalValue += childTotalValue
	}
	//fmt.Printf("Tree %s has partial weight %d and total weight %d\n", currTree.Name, currTree.Value, totalValue)
	return totalValue
}
