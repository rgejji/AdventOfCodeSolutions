...
fn iterate_states(curr_states:&mut Vec<State>, nodes:&Vec<Node>, pressures:&Vec<usize>, name_to_ind:&HashMap<String,usize>){
    ...
    */
    ...
    for _i in 0..end_time/2 {
        iterate(curr_states, &mut alt_states, nodes, pressures, max_valves.clone());
        max_valves = get_max_valves(&alt_states);
        print_highest_total(&alt_states);
        //println!("Test state is at {:?}", curr_states[debug_ind]);

        iterate(&mut alt_states, curr_states, nodes, pressures, max_valves.clone());
        max_valves = get_max_valves(&alt_states);
...
fn iterate(curr_states:&mut Vec<State>, next_states:&mut Vec<State>, nodes:&Vec<Node>, pressure:&Vec<usize>, max_valves:usize){
    let threshold = 5; //parameter we choose on how far behind states are in valves before we unvisit them.
    ...
    for (i,s) in curr_states.into_iter().enumerate() {
        ...
        if max_valves >= (s.history.count_ones() + threshold).try_into().unwrap() {
            s.visited = false;
            continue;
        }
...
fn get_max_valves(curr_states:&Vec<State>) -> usize{
    let mut m = 0;
    for s in curr_states{
        if s.visited {
            let ones = s.history.count_ones();
            if  ones > m {
                m=ones;
            }
        }
    }
    println!("MAx valve count is {}", m);
    m.try_into().unwrap()
}
