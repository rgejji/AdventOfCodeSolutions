...
const (
	inputStr = `bdwdjjo avricm cjbmj ran lmfsom ivsof
...
func main() {
	...
	for _, row := range rowSlice {
		if isUnique := isValid(strings.Fields(row)); isUnique {
			numValid++
		}
	}
	fmt.Printf("vim-go %d\n", numValid)
...
func isValid(words []string) bool {
	...
	count := 0
	for _, unSortedWord := range words {

		//Anagrams have 1-1 in character order, we sort the word
		wordSlice := sort.StringSlice(strings.Split(unSortedWord, ""))
		wordSlice.Sort()
		word := strings.Join(wordSlice, "")

		if _, ok := test[word]; ok {
			return false
		}
		test[word] = word
		count++
