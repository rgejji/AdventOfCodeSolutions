...
fn observe_monkeys(input:&str)-> Vec<Monkey> {
	...
	while let Some(line) = line_itr.next(){
        ...
        let items:Vec<u64> = match item_split.get(1) {
			None => {
				Vec::new()
			},
			Some(i) => {
                i.split(", ").map(|s| s.parse().unwrap()).collect::<Vec<u64>>()
            }
		};
	    //get operation
		let line = line_itr.next().unwrap();
...
//Add tracking by lcm since we operate within this ring
fn monkey_move(monkeys: &mut Vec<Monkey>, counts: &mut Vec<u64>, lcm: u64) {

    for i in 0..monkeys.len() {
        let monkey = &monkeys[i];
        let true_index = monkey.true_monkey;
        let false_index = monkey.false_monkey;
	    let mut true_items = Vec::<u64>::new();
    	let mut false_items = Vec::<u64>::new();

        for item in monkey.items.iter(){
            let term:u64 = match monkey.operator_val {
                SameOrU32::U(i) => i,
                SameOrU32::Same => *item
            };
...
fn get_num_from_line(line:&str, index:usize, err:&str) -> u64 {
...
fn main() {
    ...
    let mut lcm = monkeys[0].test_div;
    for i in 1..monkeys.len(){
        lcm = lcm*monkeys[i].test_div/(lcm.gcd(monkeys[i].test_div));
    }
    println!("Calculated lcm is {}", lcm);

    for _i in 0..NUM_ROUNDS {
        monkey_move(&mut monkeys, &mut counts, lcm);
    }
