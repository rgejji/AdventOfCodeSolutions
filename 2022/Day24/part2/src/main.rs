...
fn main() {
    ...
    let mut begin:State = State(start.0, start.1, 0);
    let mut result = astar(&begin, |p| p.weighted_successors(&master_array), |p| p.distance(&goal), |p| p.0 == goal.0 && p.1 == goal.1);
    let first = result.expect("no S-> E path").1;
    begin = State(goal.0, goal.1, first);
    result = astar(&begin, |p| p.weighted_successors(&master_array), |p| p.distance(&start), |p| p.0 == start.0 && p.1 == start.1);
    let second = first + result.expect("no E-> S path").1;
    begin = State(start.0, start.1, second);
    result = astar(&begin, |p| p.weighted_successors(&master_array), |p| p.distance(&goal), |p| p.0 == goal.0 && p.1 == goal.1);
    let third = second + result.expect("no E-> S path").1;
    println!("Path took {} steps", third);
